...
output "frontend_service_name" {
  ...
  value       = aws_service_discovery_service.frontend.name
}

output "frontend_dns_name" {
  description = "Fully qualified DNS name for frontend service (for Prometheus config)"
  value       = "${aws_service_discovery_service.frontend.name}.${aws_service_discovery_private_dns_namespace.main.name}"
}
...
output "prometheus_dns_sd_configs" {
  ...
  value = {
    backend = {
      dns_name = "${aws_service_discovery_service.backend.name}.${aws_service_discovery_private_dns_namespace.main.name}"
      port     = 3001 # Default backend port - override if different
      type     = "SRV"
    }
    frontend = {
      dns_name = "${aws_service_discovery_service.frontend.name}.${aws_service_discovery_private_dns_namespace.main.name}"
      port     = 3000 # Default frontend port - override if different
      type     = "SRV"
    }
  }
}
