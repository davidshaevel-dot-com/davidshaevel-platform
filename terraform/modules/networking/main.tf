...
# ------------------------------------------------------------------------------
# Observability Tier - Prometheus Security Group
# ------------------------------------------------------------------------------
# Security group for Prometheus metrics collection service
# Allows access to metrics endpoints on backend and frontend

resource "aws_security_group" "prometheus" {
  name_prefix = "${var.environment}-${var.project_name}-prometheus-sg-"
  description = "Security group for Prometheus metrics collection - allows access to application metrics endpoints"
  vpc_id      = aws_vpc.main.id

  # NOTE: All rules are managed by separate aws_vpc_security_group_*_rule resources
  tags = merge(var.common_tags, {
    Name = "${var.environment}-${var.project_name}-prometheus-sg"
    Tier = "observability"
  })
...
# Allow outbound HTTPS to internet (for updates, external integrations)
resource "aws_vpc_security_group_egress_rule" "prometheus_to_internet" {
  security_group_id = aws_security_group.prometheus.id

  description = "Allow HTTPS to internet for updates and integrations"
  from_port   = 443
  to_port     = 443
  ip_protocol = "tcp"
  cidr_ipv4   = "0.0.0.0/0"
